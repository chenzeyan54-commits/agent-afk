...
```
~/.afk/state/witness/<session-label>/trace.jsonl
```

The `<session-label>` defaults to a UUID but can be a human-readable name when the caller supplies one (for example, when resuming a known session). `afk trace list` shows the label for each recorded session.
...
Prints `<timestamp>  <session-label>` rows. Pass `-n` / `--max` to cap the output (default 20, max 200):
...
afk trace show <session>  # a specific session label or id
```
...
```bash
tail -f ~/.afk/state/witness/<session-label>/trace.jsonl | jq .
```
