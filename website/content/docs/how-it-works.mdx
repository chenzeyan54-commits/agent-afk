...
- **`bash`, `read_file`, `write_file`, `edit_file`, `glob`, `grep`, `list_directory`** — standard agentic tools
- **`web_scrape`** — one tool, three modes: fetch a page as markdown, fetch the untransformed response text, or run a web search. There is no separate `WebFetch`/`WebSearch` — those names are aliases that normalize to `web_scrape`.
- **`browser_open`, `browser_observe`, `browser_act`, `browser_screenshot`, `browser_close`** — Playwright-backed browser control (navigate, observe, click/fill, screenshot)
...
- **`send_telegram`** — pushes a terminal-state notification to the operator; no-op if Telegram
  is unconfigured
- **`config_get` / `config_set`** — read and edit AFK's own config (`~/.afk/config/`); secrets are
  always masked and credential/safety keys are human-gated (see [Editing Configuration](/configuration/editing-config))
...
```
~/.afk/
  config/           afk.env, afk.config.json, mcp.json, schedules.json
  state/            ($AFK_STATE_DIR overrides this tier)
    sessions/       session-store sidecars + per-session event ledgers
    ...
    witness/        per-session trace/evidence files (trace.jsonl, prompts/, outputs/)
    bg/             background job logs (one subdir per job)
    queue/          pull-mode task queue
  plugins/          user-installed plugins
  skills/           user-authored and generated skills
  farms/            Speculative Branch Farm worktrees
  logs/
  cache/
  agent-framework/  telemetry, pattern cards, briefs (JSONL)
```
