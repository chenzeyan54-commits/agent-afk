...
If you already have Claude Code installed, `claude login` also works — AFK reads the credential Claude Code writes to your system keychain.
...
The default model is the `medium` capability tier — Claude Sonnet on an
unconfigured install. Override per-command:
...
- **[How It Works](/how-it-works)** — session lifecycle, provider routing, terminal states
- **[Interactive REPL](/surfaces/repl)** — slash commands, plan mode, background tasks
- **[One-Shot Chat](/surfaces/chat)** — pipe-friendly, headless streaming
- **[Daemon](/surfaces/daemon)** — cron scheduling, unattended runs
- **[Telegram](/surfaces/telegram)** — bot setup and supervision
