...
| `AFK_MAX_OUTPUT_TOKENS` | number | Cap on output tokens per turn. When unset, falls back to the model output ceiling (AFK's own per-model limit, 64k–128k — not a small provider default). An over-ceiling value is clamped down with a warning. | — |
| `AFK_MAX_TOKENS` | number | **Deprecated / inert** — not read by the generation path. Use `AFK_MAX_OUTPUT_TOKENS` to cap per-response output tokens. | `4096` |
| `AFK_MODEL` | string | Default model for agent turns. Accepts slot names (local, small, medium, large), fixed-identity aliases (opus, sonnet, haiku, fable), or full model IDs. Migration: AFK_MODEL=sonnet now pins the fixed Sonnet identity rather than following a rebound medium tier. | `medium` |
...
| `AFK_MODEL_LOCAL` | string | Bind the "local" capability tier (cheapest/fastest, user-configured) to a model id. Overrides afk.config.json models.local. Point at a local Ollama, LM Studio, or any OpenAI-compatible shim. | — |
| `AFK_MODEL_LOCAL_API_KEY` | string | Per-slot API key for the "local" tier. Overrides global credentials for this tier only. | — |
| `AFK_MODEL_LOCAL_BASE_URL` | string | Per-slot endpoint base URL for the "local" tier. Anthropic Messages base or OpenAI-compatible base per the tier provider. | — |
...
| `AFK_THINKING` | string | Extended-thinking mode. Accepts adaptive \| disabled \| max \| enabled:&lt;N&gt; \| enabled:max. Defaults to the model-appropriate mode when unset (adaptive on current models). | `adaptive` |
...
| `AFK_BROWSER_BACKEND` | string | Browser provider backend. `auto` (default) prefers Agent Browser and falls back to Playwright. `agent-browser` requires Agent Browser (fails if unavailable). `playwright` uses headless Playwright only. | `auto` |
...
| `AFK_BROWSER_DEFAULT_PROFILE` | string | Name of the persistent session-vault profile the agent reuses for browser sessions. The context restores its login from (and saves it back to) ~/.afk/state/browser/&lt;profile&gt;/storageState.json, so a human runs `afk browser login --profile <name>` once and the agent reuses that authenticated session across unattended runs. Unset defaults to `default` (a fresh, empty profile). Allowed charset: [A-Za-z0-9_-], max 128 chars. | `default` |
| `AFK_BROWSER_DOM_SNAPSHOTS` | boolean | When set to 1, every browser_act writes a gzipped DOM snapshot sidecar under ~/.afk/state/witness/&lt;sid&gt;/browser/dom-snapshots/. Off by default because snapshots are large; useful for post-mortem analysis of failed actions. | — |
| `AFK_BROWSER_HEADLESS` | boolean | Override the default headless mode for native browser-control tools. `1`/`true` forces headless; `0`/`false` forces headed. When unset the default is headless on every AFK surface — the CLI entrypoint reports surface `afk`, which is in the headless set, so the REPL is headless by default. Headed is opt-in via `AFK_BROWSER_HEADLESS=0`. Note: headed and headless use different Chromium downloads (`chromium-*` vs `chromium_headless_shell-*`). | — |
...
| Variable | Type | Description | Default |
| --- | --- | --- | --- |
| `AFK_ALLOW_PROJECT_MCP` | boolean | Opt-in to loading + spawning MCP servers declared in &lt;cwd&gt;/.mcp.json. Fail-closed: unset (or `0`) means project-local servers are **not** spawned; set to a truthy value (`1`/`true`/`yes`/`on`) to load them. Off by default because a project-local `.mcp.json` spawns arbitrary commands on session start (issue #571). | off (opt-in) |
...
| `AFK_WEB_HOST` | string | Bind address for the `afk web` browser surface. Defaults to `127.0.0.1`. A non-loopback bind is refused unless `AFK_WEB_TOKEN` (or `--token`) is also set. Overridden by `--host`. | `127.0.0.1` |
| `AFK_WEB_PORT` | number | Port for the `afk web` browser surface. Defaults to `4141`; falls back to an ephemeral port when taken. Overridden by `--port`. | `4141` |
...
| `AFK_MEMORY_EVIDENCE_GATE` | boolean | Evidence gate for durable memory writes. When enabled, a codebase fact (memory_update category "convention") stored without an evidence citation is recalled as [unverified], and memory_search results carry a verification verdict. User preferences and agent reflections are never gated. On by default. Set to `0` to disable. | `1` |
...
| `AFK_STREAM_BUFFER_MS` | number | Input buffer window for TUI streaming in milliseconds. Incoming tokens are micro-batched before parsing and rendering, producing smoother visual output. The first token after idle always passes through immediately. `0` = disabled (every token is parsed individually). Reasonable range: 8–50. | `0` |
...
| `AFK_WAVE_RESUME_UNATTENDED` | boolean | When set to `1`, surface wave resumption offers even on non-interactive surfaces (daemon, one-shot chat). By default, offers are only made on interactive surfaces (REPL, Telegram). | `0` |
