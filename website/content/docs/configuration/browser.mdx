...
the hood. In REPL mode, Chromium is headless by default — the CLI entrypoint
reports surface `afk`, which is in the headless set. Set `AFK_BROWSER_HEADLESS=0`
to open a visible window and watch the agent work.
...
|----------|---------|-------------|
| `AFK_BROWSER_HEADLESS` | headless | `1` forces headless; `0` forces headed. Defaults to headless on every surface — the CLI entrypoint reports surface `afk`, so the REPL is headless too. Headed is opt-in via `0`. Note the two modes use different chromium downloads (`chromium-*` vs `chromium_headless_shell-*`), so a cache that satisfies one can fail the other. |
...
| `AFK_BROWSER_BACKEND` | `auto` | Browser provider backend. `auto` (default) prefers Agent Browser and falls back to Playwright. `agent-browser` requires Agent Browser (fails if unavailable). `playwright` uses headless Playwright only. |
