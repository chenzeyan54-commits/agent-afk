...
`~/.afk/agent-framework/` independently — no shared state.
...
|-------|------|---------|-------------|
| `model` | `string` | `"medium"` (CLI) / `"sonnet"` (Telegram) | Default model — varies by surface. CLI commands resolve via `getModel()`, defaulting to the **`medium` capability tier** (Claude Sonnet on an unconfigured install; rebinding `models.medium` changes the CLI default). Surfaces that read `loadConfig().model` directly instead — notably the **Telegram bot** — fall back to the literal `DEFAULT_CONFIG.model` (`"sonnet"`), so rebinding `medium` alone does not affect them. Accepts tier names (`small`/`medium`/`large`), fixed-identity aliases (`sonnet`, `haiku`, `opus`, `fable`), and full provider-native ids (`gpt-4o`, `mlx-community/Qwen3-30B-A3B-4bit`). Env override: `AFK_MODEL`. Migration: `AFK_MODEL=sonnet` now pins the fixed Sonnet identity rather than following a rebound `medium` tier. |
...
Bind specific models to the `local`, `small`, `medium`, and `large` quality tiers used
...
|-------|------|-------------|
| `local` | `string \| object` | Model for locally-served inference (e.g. an MLX or llama.cpp shim). Unset by default — an unconfigured `local` slot is skipped rather than falling back. Env override: `AFK_MODEL_LOCAL`. |
...
| `provider` | `string` | Provider name: `"anthropic"`, `"openai"`, or `"chatgpt-oauth"` (forces per-slot ChatGPT-subscription OAuth — see [Models & Providers](/configuration/models-providers#chatgpt-oauth-per-slot)). Inferred from `id` when omitted. |
