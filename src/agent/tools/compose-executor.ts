...
import { parseComposeInput, type ComposeInput } from './compose-executor.parse.js';
    ...
    try {
      ...
      const dagNodes: SubagentDAGNode[] = parsed.nodes.map((n, i) => {
        ...
        return {
          ...
          // Per-node filesystem scope grants (#1945). Validation (absolute paths,
          // no `..`, not-too-broad) is handled by validateDagNodeRoots in
          // dag-subagent.ts; additive semantics (child >= parent) enforced by
          // resolveReadScope in subagent.ts at fork time.
