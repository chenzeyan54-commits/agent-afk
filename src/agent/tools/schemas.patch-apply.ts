...
export const patchApplyTool: AnthropicToolDef = {
  ...
  description:
    ...
    'On successful apply, the result includes a `_reread_warning` field listing modified paths — ' +
    'callers should re-read those files before any subsequent `edit_file` call.',
  input_schema: {
    type: 'object',
    properties: {
      changes: {
